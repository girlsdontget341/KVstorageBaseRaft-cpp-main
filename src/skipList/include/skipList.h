...
// Class template to implement node
template <typename K, typename V>//实现序列化
class SkipListDump {
 public:
  friend class boost::serialization::access;

  template <class Archive>
  void serialize(Archive &ar, const unsigned int version) {//ar指序列化对象
    ar &keyDumpVt_;
    ar &valDumpVt_;
  }
  std::vector<K> keyDumpVt_;//键集合
  std::vector<V> valDumpVt_;//值集合

 public:
  void insert(const Node<K, V> &node);//将node的key和val插入上述集合中
};
...
class SkipList {
 public:
  ...
  std::string dump_file();//序列化操作
  void load_file(const std::string &dumpStr);//加载数据
  //递归删除节点
  void clear(Node<K, V> *);
  int size();
